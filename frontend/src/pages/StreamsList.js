...
const StreamsList = () => {
  ...
  const fetchStreams = useCallback(async () => {
    try {
      ...
      // Add a unique timestamp to each stream with a screenshot to prevent caching issues
      const updatedStreams = response.data.map(newStream => {
        // If the stream has a screenshot, add a unique timestamp
        if (newStream.screenshotPath) {
          return {
            ...newStream,
            // Ensure the timestamp is always unique
            screenshotTimestamp: newStream.screenshotTimestamp || Date.now()
          };
        }
  ...
  const fetchStreamDiagnostics = useCallback(async (streamId) => {
    // Set loading state for this stream's diagnostics
    ...
    try {
      console.log(`Fetching diagnostics for stream ${streamId}...`);
      const response = await axios.get(`/api/streams/${streamId}/diagnostics`);
      console.log(`Received diagnostics data:`, response.data);
      
      // Process the diagnostics data
      const diagnostics = response.data;
      
      // Calculate derived values for better display
      const processedData = {
        ...diagnostics,
        isLoading: false,
        error: null,
        // Extract error counts by type
        networkErrors: diagnostics.errors?.byType?.network || 0,
        sourceErrors: diagnostics.errors?.byType?.source || 0,
        ffmpegErrors: diagnostics.errors?.byType?.ffmpeg || 0,
        systemErrors: diagnostics.errors?.byType?.system || 0,
        // Total error count
        errorCount: diagnostics.errors?.total || 0,
        // Recent errors for display
        errorHistory: diagnostics.errors?.recent || [],
        // Last error type
        lastErrorType: diagnostics.errors?.recent?.[0]?.type || 'None',
        // Health check status
        healthCheckStatus: diagnostics.lastHealthCheck ? 'Completed' : 'Pending',
        // Segment information
        segmentCount: diagnostics.segmentCount || 0,
        segmentGaps: diagnostics.segmentGaps || 0,
        // Last updated timestamp
        lastUpdated: new Date().toISOString()
      };
      
      console.log(`Processed diagnostics data:`, processedData);
      
      setDiagnosticsData(prev => ({
        ...prev,
        [streamId]: processedData
      }));
    } catch (error) {
      ...
      setDiagnosticsData(prev => ({
        ...
        [streamId]: {
          ...(prev[streamId] || {}),
          isLoading: false,
          error: error.response?.data?.error || error.message || 'Failed to load diagnostics'
        }
      }));
    }
  }, []);
  ...
  const handleStreamExpand = (streamId) => {
    if (expandedStreamId === streamId) {
      ...
      // Always fetch fresh diagnostics when expanding
      fetchStreamDiagnostics(streamId);
    }
  };
  ...
  const handleTakeScreenshot = async (streamId) => {
    try {
      // Find the stream to get its name for better error messages
      const stream = streams.find(s => s.id === streamId);
      const streamName = stream ? stream.name : 'Unknown stream';
      
      setSnackbar({
        open: true,
        message: `Taking screenshot for ${streamName}...`,
        severity: 'info'
      });
      
      // First, clear the existing screenshot to prevent the flickering effect
      setStreams(prevStreams => 
        prevStreams.map(stream => 
          stream.id === streamId 
            ? {
                ...stream,
                // Keep the path but invalidate it with a temporary timestamp
                screenshotTimestamp: `pending-${Date.now()}`
              } 
            : stream
        )
      );
      ...
      const timestamp = response.data.screenshotTimestamp || Date.now();
      ...
      // Update the streams with the new screenshot path and timestamp
      setStreams(prevStreams => 
        prevStreams.map(stream => 
          stream.id === streamId 
            ? {
      ...
      // No need to force a refresh since we're already updating the state properly
      ...
      if (error.response) {
        ...
        errorMessage = 'No response from server when taking screenshot';
      } else {
      ...
      {loading ? (
        ...
        <Card 
          sx={{ 
            p: 4, 
            textAlign: 'center', 
            background: 'linear-gradient(145deg, #1e1e1e 0%, #121212 100%)', // Match theme.js MuiCard style
            borderRadius: '16px',
          ...
          {streams.map((stream) => (
            <Grid item xs={12} sm={streams.length === 1 ? 10 : 8} md={streams.length === 1 ? 8 : 6} key={stream.id} sx={streams.length === 1 ? { mx: 'auto' } : {}}>
              <Fade in={true} timeout={500}>
                <Card 
                  sx={{ 
                    backgroundColor: 'rgba(0, 0, 0, 0.2)', // Updated to match the dark theme
                    borderRadius: '16px',
                  ...
                  <CardContent sx={{ p: 2, pt: 1, pb: 1, flexGrow: 1 }}>
                    ...
                    <Box sx={{ 
                      ...
                      {stream.screenshotPath ? (
                        ...
                        >
                          <img
                            src={`${stream.screenshotPath}${stream.screenshotPath.includes('?') ? '&' : '?'}t=${stream.screenshotTimestamp || new Date().getTime()}&nocache=${Date.now()}`}
                            key={stream.screenshotTimestamp || 'initial'}
                  ...
                  >
                    <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                      <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                        ...
                        {expandedStreamId === stream.id && diagnosticsData[stream.id]?.isLoading && (
                          <CircularProgress size={16} sx={{ ml: 2 }} />
                        )}
                      </Box>
                    </AccordionSummary>
                    <AccordionDetails>
                      {diagnosticsData[stream.id]?.isLoading ? (
                        <Box sx={{ display: 'flex', justifyContent: 'center', p: 2 }}>
                          <CircularProgress size={24} />
                        </Box>
                      ) : diagnosticsData[stream.id]?.error ? (
                        <Box sx={{ p: 2 }}>
