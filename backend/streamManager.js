...
class StreamManager {
  ...
  takeScreenshot(id) {
    this.log('info', `Attempting to take screenshot for stream ${id}`);
    
    if (!this.streams[id]) {
      this.log('error', `Cannot take screenshot: Stream ${id} not found`);
      return false;
    }
    
    if (this.streams[id].status !== 'running') {
      this.log('error', `Cannot take screenshot: Stream ${id} (${this.streams[id].name}) is not running, status: ${this.streams[id].status}`);
      return false;
    }

    const screenshotPath = path.join(this.screenshotsDir, `${id}.jpg`);
    this.log('info', `Screenshot will be saved to: ${screenshotPath}`);
    
    // Ensure the screenshots directory exists
    if (!fs.existsSync(this.screenshotsDir)) {
      this.log('info', `Creating screenshots directory: ${this.screenshotsDir}`);
      fs.mkdirSync(this.screenshotsDir, { recursive: true });
    }
    
    try {
      // Try multiple approaches to take a screenshot
      return this.takeScreenshotWithFFmpeg(id, screenshotPath);
    } catch (error) {
  ...
  takeScreenshotWithFFmpeg(id, screenshotPath) {
    const stream = this.streams[id];
    this.log('info', `Taking screenshot with FFmpeg for ${stream.name} (${id}) from URL: ${stream.url}`);
    
    try {
      // Enhanced FFmpeg arguments for better compatibility
      const ffmpegArgs = [
      ...
      this.log('debug', `FFmpeg command: ffmpeg ${ffmpegArgs.join(' ')}`);
      const result = spawnSync('ffmpeg', ffmpegArgs, { timeout: 20000 });
      
      if (result.status === 0) {
        // Verify the screenshot file was created
        if (fs.existsSync(screenshotPath)) {
          const fileSize = fs.statSync(screenshotPath).size;
          this.log('info', `Screenshot file created successfully: ${screenshotPath} (${fileSize} bytes)`);
          
          // Update the stream object with the screenshot path and timestamp
          stream.screenshotPath = `/api/screenshots/${id}.jpg`;
          stream.screenshotTimestamp = new Date().getTime();
          
          this.log('info', `[${stream.name}] Screenshot taken successfully`);
          return true;
        } else {
          this.log('error', `[${stream.name}] Screenshot file not created despite successful FFmpeg exit code`);
          return false;
        }
      } else {
        const stderr = result.stderr ? result.stderr.toString() : 'No error output';
        const stdout = result.stdout ? result.stdout.toString() : 'No standard output';
        this.log('error', `[${stream.name}] Failed to take screenshot: ${stderr}`);
        this.log('debug', `[${stream.name}] FFmpeg stdout: ${stdout}`);
        
        // Try a fallback approach with different FFmpeg options
        return this.takeScreenshotWithFFmpegFallback(id, screenshotPath);
      }
    } catch (error) {
      this.log('error', `[${stream.name}] Error in takeScreenshotWithFFmpeg: ${error.message}`);
      return this.takeScreenshotWithFFmpegFallback(id, screenshotPath);
    }
  }
  
  // Fallback method with different FFmpeg options
  takeScreenshotWithFFmpegFallback(id, screenshotPath) {
    const stream = this.streams[id];
    this.log('info', `Trying fallback screenshot method for ${stream.name} (${id})`);
    
    try {
      // Simpler FFmpeg arguments as a fallback
      const ffmpegArgs = [
        '-y',
        '-i', stream.url,
        '-vframes', '1',
      ...
      this.log('debug', `FFmpeg fallback command: ffmpeg ${ffmpegArgs.join(' ')}`);
      const result = spawnSync('ffmpeg', ffmpegArgs, { timeout: 15000 });
      ...
      if (result.status === 0 && fs.existsSync(screenshotPath)) {
        ...
        this.log('info', `[${stream.name}] Screenshot taken successfully with fallback method`);
        return true;
      } else {
        const stderr = result.stderr ? result.stderr.toString() : 'No error output';
        this.log('error', `[${stream.name}] Failed to take screenshot with fallback method: ${stderr}`);
        return false;
      }
    } catch (error) {
      this.log('error', `[${stream.name}] Error in takeScreenshotWithFFmpegFallback: ${error.message}`);
      return false;
    }
  }
