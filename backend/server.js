...
// Custom morgan logging to reduce noise from 304 responses
app.use(morgan((tokens, req, res) => {
  // Skip logging 304 responses for /api/streams to reduce log spam
  if (req.path === '/api/streams' && res.statusCode === 304) {
    return null; // Skip logging this request
  }
  
  // Standard dev format for other requests
  return [
...
app.post('/api/streams/:id/screenshot', (req, res) => {
  console.log(`[Screenshot] Received request for stream ID: ${req.params.id}`);
  
  // Check if stream exists first
  const stream = streamManager.getStream(req.params.id);
  if (!stream) {
    console.log(`[Screenshot] Stream not found: ${req.params.id}`);
    return res.status(404).json({ error: 'Stream not found' });
  }
  
  console.log(`[Screenshot] Taking screenshot for stream: ${stream.name} (${req.params.id})`);
  const success = streamManager.takeScreenshot(req.params.id);
  
  if (!success) {
    console.log(`[Screenshot] Failed to take screenshot for stream: ${stream.name} (${req.params.id})`);
    return res.status(404).json({ error: 'Could not take screenshot' });
  }
  
  // Ensure we have a valid path and timestamp
  if (!stream.screenshotPath) {
    stream.screenshotPath = `/api/screenshots/${req.params.id}.jpg`;
  }
  
  if (!stream.screenshotTimestamp) {
    stream.screenshotTimestamp = new Date().getTime();
  }
  
  console.log(`[Screenshot] Successfully took screenshot for stream: ${stream.name} (${req.params.id})`);
  console.log(`[Screenshot] Path: ${stream.screenshotPath}, Timestamp: ${stream.screenshotTimestamp}`);
